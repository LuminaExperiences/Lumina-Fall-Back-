...
function generateAndSendTickets(rowNumber) {
  ...
  try {
    ...
    const baseId = `BIGFAKE${rowNumber}${Date.now().toString().slice(-5)}`; // Unique base ID
...
/** Gets shared secret from Config sheet */
function getSharedSecret(configSheet) {
  try {
    if (!configSheet) configSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG_SHEET_NAME);
    if (!configSheet) throw new Error(`Sheet "${CONFIG_SHEET_NAME}" not found.`);
    
    const secret = configSheet.getRange('B3').getValue().toString().trim();
    if (!secret) {
      Logger.log(`Warning: No shared secret found in ${CONFIG_SHEET_NAME}!B3.`);
      return null;
    }
    return secret;
  } catch (error) {
    Logger.log(`ERROR getting shared secret: ${error}.`);
    return null;
  }
}

/** Inserts custom form submission into sheet and triggers existing workflow */
function insertCustomFormSubmission(fullName, email, numTickets, attendeeNamesStr, screenshotUrl) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = ss.getSheetByName(RESPONSES_SHEET_NAME);
    
    if (!sheet) {
      Logger.log(`ERROR: Sheet '${RESPONSES_SHEET_NAME}' not found`);
      return false;
    }
    ...
    // Insert data in same format as Google Form
    sheet.getRange(nextRow, COL_TIMESTAMP).setValue(timestamp);
    sheet.getRange(nextRow, COL_FULL_NAME).setValue(fullName);
    sheet.getRange(nextRow, COL_PAYER_EMAIL).setValue(email);
    sheet.getRange(nextRow, COL_NUM_TICKETS).setValue(parseInt(numTickets));
    sheet.getRange(nextRow, COL_ATTENDEE_NAMES).setValue(attendeeNamesStr || fullName);
    sheet.getRange(nextRow, COL_SCREENSHOT).setValue(screenshotUrl || '');
    
    Logger.log(`Inserted custom form data at row ${nextRow}`);
    
    // Create fake form submit event to trigger existing logic
    const fakeEvent = {
      range: sheet.getRange(nextRow, 1, 1, sheet.getLastColumn()),
      namedValues: {
        'Timestamp': [timestamp.toString()],
        'Full Name': [fullName],
        'Email Address (Non-UW only)': [email],
        'How many people are you booking for?': [numTickets.toString()],
        'If you are booking just for yourself, kindly enter your name. If you are booking tickets for other people as well, kindly enter their names, separated by commas.': [attendeeNamesStr || fullName]
      }
    };
    
    // Call existing onFormSubmit logic
    onFormSubmit(fakeEvent);
    
    return true;
    
  } catch (error) {
    Logger.log(`ERROR in insertCustomFormSubmission: ${error}\nStack: ${error.stack}`);
    return false;
  }
}
...
<p>Hi ${fullName},</p><p>Great news! Your payment has been confirmed. ✅ Please find your ${attendeeNames.length} ticket(s) for The Big Fake Indian Wedding below.</p><p><strong>Important:</strong> Each QR code below represents one entry. Please have them ready for scanning at the door on your phone.</p><hr>`;}]}}}
  ...
  htmlBody += `<hr><p><strong>Event Details Recap:</strong></p><p>Event: The Big Fake Indian Wedding<br>Date: May 17th, 2025<br>Time: 11:00 PM onwards<br>Venue: 1218 10th Avenue, Capitol Hill, Seattle, Washington</p><p>See you at the celebration!</p><p>Cheers,<br>UW Lumina Team</p></body></html>`;
  try {
...
/** [WEB APP] Handles POST requests from custom frontend */
function doPost(e) {
  try {
    ...
    const { fullName, email, numTickets, attendeeNamesStr, screenshotUrl, secret } = requestData;
    
    if (!fullName || !email || !numTickets || !secret) {
      return ContentService
    ...
    const expectedSecret = getSharedSecret(configSheet);
    
    if (secret !== expectedSecret) {
      Logger.log(`Security validation failed. Expected: ${expectedSecret}, Got: ${secret}`);
      return ContentService
    ...
    const success = insertCustomFormSubmission(fullName, email, numTickets, attendeeNamesStr, screenshotUrl);
